...
export const ListJobs = (jobData) => {
  ...
  useEffect(() => {
    if (job_order.length !== 0) {
      // TODO: write message?
      axios
        .put("/sortupdate", { job_order }, { withCredentials: true })
        .then((response) => {});
    }
  }, [job_order]);
  ...
  const NewJobModal = () => {
    ...
    const finishJob = () => {
      ...
      axios
        .put("/finishjob", { item }, { withCredentials: true })
        .then((response) => {});
    };
              ...
              {job_order.map((item, index) => {
                return (
                  <Draggable
                    ...
                    draggableId={item.job_id.toString()}
                    index={index}
                  >
                    {(provided) => (
                      <ul
                        className="jobItem"
                        onClick={() => {
                          setModal({ state: true, activeItem: item });
                        }}
                        ref={provided.innerRef}
                      ...
                      >
                        <div className="jobInfo">
                          ...
                          <div className="addr">
                            <a
                              href={"https://maps.google.com/?q=" + item.addr}
                              onClick={(e) => {
                                e.stopPropagation();
                              }}
                            >
